...
def load_cifar10(args):
    ...
    original_train = datasets.CIFAR10(root=args.data_dir, train=True, download=True, transform=transforms.ToTensor())
    ...
    test_dataset = datasets.CIFAR10(root=args.data_dir, train=False, transform=test_transform)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=args.batch_size*4, shuffle=False, pin_memory=True, num_workers=args.workers)
    ...
    if args.exp_type == 'rand':
        ...
        model.load_state_dict(torch.load(os.path.join(args.data_dir, './pmodel/cifar10.pt')))
    ...
    unlabeled_training_dataset = CIFAR10_Unlabeled(unlabeled_data, unlabeled_gt)
